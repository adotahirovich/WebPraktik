...
function validate_user_registration()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (strlen($full_name) < 3) {
            $errors[] = "Your Full Name cannot be less then 3 characters";
        }
...
function create_user($full_name,  $username, $email, $password)
{
    ...
    $sql = "INSERT INTO users(full_name,username,profile_image,email,password) ";
    $sql .= "VALUES('$full_name','$username','uploads/default.jpg','$email','$password')";
    confirm(query($sql));
...
function fetch_all_posts()
{
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div class='post'><p><img src='" . $user['profile_image'] . "' alt=''><i><b>" . $user['full_name'] . " " . "</b></i></p>
                    <p>" . $row['content'] . "</p>
                    <p><i>Date: <b>" . $row['created_time'] . "</b></i></p>
                    <div class='likes'>Likes: <b id='likes_".$row['id']."'>" . $row['likes'] . "</b><button onclick='like_post(this)' data-post_id='".$row['id']."'>LIKE</button></div>
                    <div class='unlikes'>Unlikes: <b id='unlikes_".$row['id']."'>" . $row['unlikes'] . "</b><button onclick='unlike_post(this)' data-post_id='".$row['id']."'>UNLIKE</button></div>
                    </div>";
        }
    }
}
