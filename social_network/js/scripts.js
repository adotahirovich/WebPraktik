...
function unlike_post(el) {
    ...
    xmlhttp.onload = function () {
        let id = el.getAttribute('data-post_id');
        document.getElementById('unlikes_' + id).innerText = this.responseText;

        el.setAttribute('disabled', 'disabled');

        setCookie("unliked", el.getAttribute('data-post_id'), 9999);
    }

    xmlhttp.open("GET", "functions/unlike.php?post_id=" + el.getAttribute('data-post_id'));
    xmlhttp.send();
}

//if(getCookie("unliked") !== "") {
 //   document.querySelector('button[data-post_id="'+getCookie("unliked")+'"]').setAttribute('disabled', 'disabled');
//}
